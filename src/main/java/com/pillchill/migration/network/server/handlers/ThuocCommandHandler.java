            ...
            return switch (ThuocCM.valueOf(action)) {
                ...
                case CREATE -> {
                    if (!(request.getData() instanceof ThuocPayload payload)) {
                        yield Response.error("Payload tạo thuốc không hợp lệ");
                    }
                    Thuoc thuoc = toThuocEntity(payload);
                    thuocService.createThuoc(thuoc, payload.getGiaBanDau() == null ? 0.0 : payload.getGiaBanDau());
                    yield Response.success(null, "Thêm thuốc thành công");
                }
                case UPDATE -> {
                    if (!(request.getData() instanceof ThuocPayload payload)) {
                        yield Response.error("Payload cập nhật thuốc không hợp lệ");
                    }
                    Thuoc thuoc = toThuocEntity(payload);
                    thuocService.updateThuoc(thuoc);
                    yield Response.success(null, "Cập nhật thuốc thành công");
                }
                case DELETE -> {
                    if (!(request.getData() instanceof String maThuoc) || maThuoc.isBlank()) {
                        yield Response.error("Payload xóa thuốc không hợp lệ");
                    }
                    boolean result = thuocService.deactivateThuoc(maThuoc);
                    if (!result) {
